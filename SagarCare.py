# hospital_system_full.py
# FULL UPDATED HOSPITAL MANAGEMENT SYSTEM WITH STAFF & ROOM MANAGEMENT
# ----------------------------------------------------------------------
# Adds:
# - Doctor dropdown in New Admission (select from doctors table)
# - Room selection in New Admission (select only available rooms)
# - Room table with status and patient linkage
# - Room Availability page (Add/Edit/Delete rooms)
# - Discharge and Delete Record buttons in Admitted Patient Detail freeing rooms
# - Staff manager (add/edit/delete doctors & nurses)
...
# Admissions table (keeps doctor_name); ensure room_no column exists
cursor.execute("""
...
# Nurse treatment records (keeps nurse_name)
cursor.execute("""
...
class HospitalApp(ctk.CTk):
    def __init__(self):
       ...
       # ctk.CTkButton(self.sidebar, text="Ambulance Status", command=lambda: self.show_message("Not Implemented", "Ambulance page not in this build.")).grid(row=6, column=0, padx=12, pady=6, sticky="ew")
    ...
    def show_admission_form(self):
        ...
        # Doctor Dropdown (populate from doctors table)
    ...
    def save_admission(self):
        ...
        # Optionally add a nurse_treatment stub linking nurse to patient if selected
